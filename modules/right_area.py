...
class RightArea(widgets.QFrame):
        def __init__(self, parent: None, search, card, content_frame, main_window):
                ...
                #label left frame
                self.top_label = widgets.QLabel(container2, text = 'Current position')
                ...
                self.TODAY_LABEL = widgets.QLabel(container3, text = "Today")
                ...
                days = ["Monday", "Thuesday", "Wednesday", "Thirdsday", "Friday", "Saturday", "Sunday"]
                ...
                self.botom1_layot = widgets.QHBoxLayout()
                ...
                l_arrow_icon = gui.QIcon('media/arrows/arrow_left.png')
                arrow_left.setIcon(l_arrow_icon)
                arrow_left.setFixedSize(50,50)
                arrow_left.setStyleSheet("background-color: rgba(0,0,0,30); border: none;")
                

                #arrow right
                arrow_right = widgets.QPushButton()
                r_arrow_icon = gui.QIcon('media/arrows/arrow_right.png')
                arrow_right.setIcon(r_arrow_icon)
                arrow_right.setFixedSize(50, 50)      
                # arrow_right.setIconSize(core.QSize(90, 90))  # размер иконки (чуть меньше кнопки)
                arrow_right.setStyleSheet("background-color: rgba(0,0,0,30); border: none;")

                self.HOUR_FORECAST = Hour_forecast(parent=self.bottom1)
                self.LAYOUT.addWidget(self.bottom1, 2, 0, 1, 0)
                self.botom1_layot.addWidget(arrow_left)
                self.botom1_layot.addWidget(self.HOUR_FORECAST)
                self.botom1_layot.addWidget(arrow_right)
                #connect  arrows funk
                arrow_right.pressed.connect(self.right_arrow_pressed)
                arrow_left.pressed.connect(self.left_arrow_pressed)


                #second botton widget
        ...
        def update_listwidget_position(self):
                ...
                self.listwidget.move(pos.x(), pos.y() + 5)  # +5 небольшой отступ
        ...
        def left_arrow_pressed(self):
                # В начало
                if self.left_arrow_pressed_flag == False :
        ...
        def right_arrow_pressed(self):
                if self.right_arrow_pressed_flag == False:
                # В конец — через scrollbar
                        self.HOUR_FORECAST.SCROLL_AREA.horizontalScrollBar().setValue(self.HOUR_FORECAST.SCROLL_AREA.horizontalScrollBar().maximum())
